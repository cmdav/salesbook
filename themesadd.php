		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "themesview.php")
...
var CurrentForm = fthemesadd = new ew_Form("fthemesadd", "add");
...
fthemesadd.Lists["x__Default[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fthemesadd.Lists["x__Default[]"].Options = <?php echo json_encode($themes->_Default->Options()) ?>;
...
?>
<form name="fthemesadd" id="fthemesadd" class="<?php echo $themes_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-table="themes" data-field="x_Theme_ID" name="x_Theme_ID" id="x_Theme_ID" size="30" maxlength="25" placeholder="<?php echo ew_HtmlEncode($themes->Theme_ID->getPlaceHolder()) ?>" value="<?php echo $themes->Theme_ID->EditValue ?>"<?php echo $themes->Theme_ID->EditAttributes() ?>>
</span>
...
<button class="btn btn-danger ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $themes_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fthemesadd.Validate() == true ) { alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fthemesadd").submit(); } }).set("title", "<?php echo $Language->Phrase('AlertifyConfirm'); ?>").set("defaultFocus", "cancel").set('oncancel', function(closeEvent){ alertify.error('<?php echo $Language->Phrase('AlertifyCancel'); ?>');}).set('labels', {ok:'<?php echo $Language->Phrase("MyOKMessage"); ?>!', cancel:'<?php echo $Language->Phrase("MyCancelMessage"); ?>'}); } return false; }
</script>
<?php } ?>
