...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "stats_hourlist.php")
						$sReturnUrl = $this->AddMasterUrl($this->GetListUrl()); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "stats_hourview.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("stats_hourlist.php"), "", $this->TableVar, TRUE);
...
var CurrentForm = fstats_houradd = new ew_Form("fstats_houradd", "add");
...
?>
<form name="fstats_houradd" id="fstats_houradd" class="<?php echo $stats_hour_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-table="stats_hour" data-field="x_Year" name="x_Year" id="x_Year" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Year->getPlaceHolder()) ?>" value="<?php echo $stats_hour->Year->EditValue ?>"<?php echo $stats_hour->Year->EditAttributes() ?>>
</span>
...
<input type="text" data-table="stats_hour" data-field="x_Month" name="x_Month" id="x_Month" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Month->getPlaceHolder()) ?>" value="<?php echo $stats_hour->Month->EditValue ?>"<?php echo $stats_hour->Month->EditAttributes() ?>>
</span>
...
<input type="text" data-table="stats_hour" data-field="x_Date" name="x_Date" id="x_Date" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Date->getPlaceHolder()) ?>" value="<?php echo $stats_hour->Date->EditValue ?>"<?php echo $stats_hour->Date->EditAttributes() ?>>
</span>
...
<input type="text" data-table="stats_hour" data-field="x_Hour" name="x_Hour" id="x_Hour" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Hour->getPlaceHolder()) ?>" value="<?php echo $stats_hour->Hour->EditValue ?>"<?php echo $stats_hour->Hour->EditAttributes() ?>>
</span>
...
<input type="text" data-table="stats_hour" data-field="x_Hits" name="x_Hits" id="x_Hits" size="30" placeholder="<?php echo ew_HtmlEncode($stats_hour->Hits->getPlaceHolder()) ?>" value="<?php echo $stats_hour->Hits->EditValue ?>"<?php echo $stats_hour->Hits->EditAttributes() ?>>
</span>
...
<button class="btn btn-danger ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $stats_hour_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
...
<script type="text/javascript">
$('#btnAction').attr('onclick', 'return alertifyAdd(this)'); function alertifyAdd(obj) { <?php global $Language; ?> if (fstats_houradd.Validate() == true ) { alertify.confirm("<?php echo $Language->Phrase('AlertifyAddConfirm'); ?>", function (e) { if (e) { $(window).unbind('beforeunload'); alertify.success("<?php echo $Language->Phrase('AlertifyAdd'); ?>"); $("#fstats_houradd").submit(); } }).set("title", "<?php echo $Language->Phrase('AlertifyConfirm'); ?>").set("defaultFocus", "cancel").set('oncancel', function(closeEvent){ alertify.error('<?php echo $Language->Phrase('AlertifyCancel'); ?>');}).set('labels', {ok:'<?php echo $Language->Phrase("MyOKMessage"); ?>!', cancel:'<?php echo $Language->Phrase("MyCancelMessage"); ?>'}); } return false; }
</script>
<?php } ?>
