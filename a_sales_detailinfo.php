...
class ca_sales_detail extends cTable {
	...
	function __construct() {
		...
		// Update Table
		$this->UpdateTable = "`a_sales_detail`";
		$this->DBID = 'DB';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Sales_ID
		$this->Sales_ID = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_ID', 'Sales_ID', '`Sales_ID`', '`Sales_ID`', 3, -1, FALSE, '`Sales_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->Sales_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Sales_ID'] = &$this->Sales_ID;

		// Sales_Number
		$this->Sales_Number = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Number', 'Sales_Number', '`Sales_Number`', '`Sales_Number`', 200, -1, FALSE, '`Sales_Number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Sales_Number'] = &$this->Sales_Number;

		// Supplier_Number
		$this->Supplier_Number = new cField('a_sales_detail', 'a_sales_detail', 'x_Supplier_Number', 'Supplier_Number', '`Supplier_Number`', '`Supplier_Number`', 200, -1, FALSE, '`Supplier_Number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->fields['Supplier_Number'] = &$this->Supplier_Number;

		// Stock_Item
		$this->Stock_Item = new cField('a_sales_detail', 'a_sales_detail', 'x_Stock_Item', 'Stock_Item', '`Stock_Item`', '`Stock_Item`', 200, -1, FALSE, '`Stock_Item`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->fields['Stock_Item'] = &$this->Stock_Item;

		// Sales_Quantity
		$this->Sales_Quantity = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Quantity', 'Sales_Quantity', '`Sales_Quantity`', '`Sales_Quantity`', 5, -1, FALSE, '`Sales_Quantity`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Sales_Quantity'] = &$this->Sales_Quantity;

		// Purchasing_Price
		$this->Purchasing_Price = new cField('a_sales_detail', 'a_sales_detail', 'x_Purchasing_Price', 'Purchasing_Price', '`Purchasing_Price`', '`Purchasing_Price`', 5, -1, FALSE, '`Purchasing_Price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Purchasing_Price'] = &$this->Purchasing_Price;

		// Sales_Price
		$this->Sales_Price = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Price', 'Sales_Price', '`Sales_Price`', '`Sales_Price`', 5, -1, FALSE, '`Sales_Price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Sales_Price'] = &$this->Sales_Price;

		// Sales_Total_Amount
		$this->Sales_Total_Amount = new cField('a_sales_detail', 'a_sales_detail', 'x_Sales_Total_Amount', 'Sales_Total_Amount', '`Sales_Total_Amount`', '`Sales_Total_Amount`', 5, -1, FALSE, '`Sales_Total_Amount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "a_sales") {
			if ($this->Sales_Number->getSessionValue() <> "")
				$sMasterFilter .= "`Sales_Number`=" . ew_QuotedValue($this->Sales_Number->getSessionValue(), EW_DATATYPE_STRING, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "a_sales") {
			if ($this->Sales_Number->getSessionValue() <> "")
				$sDetailFilter .= "`Sales_Number`=" . ew_QuotedValue($this->Sales_Number->getSessionValue(), EW_DATATYPE_STRING, "DB");
			else
				return "";
		}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("a_sales_detailview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("a_sales_detailview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "a_sales_detailadd.php?" . $this->UrlParm($parm);
		else
			$url = "a_sales_detailadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("a_sales_detailedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("a_sales_detailadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "a_sales" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_Sales_Number=" . urlencode($this->Sales_Number->CurrentValue);
		}
		return $url;
	}
   ...
   // Common render codes
	...
	function Row_Rendered() {
		...
		$this->Sales_Quantity->EditAttrs["onkeyup"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotalFinal();";
		$this->Sales_Quantity->EditAttrs["onclick"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotalFinal();";
		$this->Sales_Price->EditAttrs["onkeyup"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotalFinal();";
		$this->Sales_Price->EditAttrs["onclick"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotalFinal();";
		$this->Sales_Quantity->EditAttrs["onfocus"] = "SetFocusTextGrid(event, 'Sales_Quantity');";
		$this->Sales_Total_Amount->EditAttrs["onblur"] = "CalculateGrid(event);GetAmountTotalFinal();GetBalanceTotalFinal();";
