...
// where 'product_type_id' matches, 'status' is 1, and 'branch_id' matches the authenticated user's branch ID, grouped by 'product_type_id'
->addSelect(DB::raw("
    (SELECT JSON_OBJECT(
    ...
    AND stores.status = 1 
    AND stores.branch_id = ".auth()->user()->branch_id."  -- Added condition to filter by authenticated user's branch ID
...
// where 'product_type_id' matches, 'status' is 1, 'prices.status' is 1, and 'branch_id' matches the authenticated user's branch ID
->addSelect(DB::raw("
