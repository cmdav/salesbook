...
class AutogeneratedSystemPriceRepository
{
    public function index()
    {
        $model =  AutogeneratedSystemPrice::get();
        if($model) {
    ...
    {
        $model = AutogeneratedSystemPrice::where('id', $id)->first();
        if($model) {
            return response()->json([ 'success' => true, 'message' => 'Record retrieved successfully', 'data' => $model], 200);
        }
        return response()->json([ 'success' => false, 'message' => 'No record found', 'data' => $model], 404);
    ...
    {
        try {
            $model =  AutogeneratedSystemPrice::create($data);
    ...
    {
        $model = AutogeneratedSystemPrice::findOrFail($id);
        $model->delete();
        return $model;
    }
}
