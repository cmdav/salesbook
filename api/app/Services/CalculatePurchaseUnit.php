...
class CalculatePurchaseUnit
{
    private function calculateSmallestUnit($unitId, $unitsMap, &$cache)
    {
        ...
        if (!isset($unitsMap[$unitId])) {
            return ['smallest_unit' => 1, 'purchase_unit_id' => null];
        }
        ...
        $cache[$unitId] = [
            ...
            'purchase_unit_id' => $currentUnit['parent_purchase_unit_id']
        ];
    ...
    public function calculatePurchaseUnits($measurements)
    {
        ...
        foreach ($measurements as $measurement) {
            ...
            $result[$unitName] = [
                'smallest_unit' => $calculatedUnit['smallest_unit'],
                'purchase_unit_id' => $calculatedUnit['purchase_unit_id']
            ];
        }
        ...
        $minValue = min(array_filter(array_column($result, 'smallest_unit'), function ($value) { return $value > 0; }));

        // Scale up the values to whole numbers
        $scaledResult = [];
        foreach ($result as $unitName => $unitData) {
            $scaledResult[$unitName] = [
                'value' => round($unitData['smallest_unit'] / $minValue),
